...
Future<TruckingSinglePageModel> fetchSinglePage(int id) async {
  final response = await http.post(Uri.parse(Api.apiUrl + 'trucking/$id'));

// Appropriate action depending upon the

// server response
  if (response.statusCode == 200) {
    return TruckingSinglePageModel.fromJson(json.decode(response.body));
  } else {
...
Future<HouseBoatGalleryModel> fetchGallery(int id) async {
  final response = await http.post(Uri.parse(Api.apiUrl + 'trucking/gallery/$id'));
...
Future<HouseBoatVideosModel> fetchVideos(int id) async {
  final response = await http.post(Uri.parse(Api.apiUrl + 'houseboat/videos/$id'));
...
class TruckingSinglePagePackage extends StatefulWidget {
  TruckingSinglePagePackage({Key? key}) : super(key: key);

  @override
  _TruckingSinglePagePackageState createState() => _TruckingSinglePagePackageState();
}

class _TruckingSinglePagePackageState extends State<TruckingSinglePagePackage> {
  Future<TruckingSinglePageModel>? futurePackage;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: SafeArea(
        child: Center(
          child: FutureBuilder<TruckingSinglePageModel>(
            future: futurePackage,
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                name = snapshot.data!.trucking!.name!;
                icon = DbController.updateIcon(name: snapshot.data!.trucking!.name!);
                return Stack(
                  children: <Widget>[
                            ...
                            child: Column(
                              children: [
                                Stack(
                                  clipBehavior: Clip.none,
                                  children: [
                                    snapshot.data!.trucking!.image!.isEmpty || snapshot.data!.trucking!.image == null
                                        ? Container(
                                            height: 500,
                                        ...
                                        : Container(
                                            ...
                                            decoration: BoxDecoration(
                                                image: DecorationImage(
                                                    colorFilter: ColorFilter.mode(Colors.black.withOpacity(0.4), BlendMode.darken),
                                                    image: NetworkImage(Api.imageUrl + snapshot.data!.trucking!.image.toString()),
                                                    fit: BoxFit.cover),
                                                color: Colors.black),
                                          ),
                                    Positioned(
                                                ...
                                                children: [
                                                  Container(
                                                    ...
                                                    child: Text(
                                                      ...
                                                      snapshot.data!.trucking!.name.toString(),
                                                      maxLines: 1,
                                              ...
                                              InkWell(
                                                onTap: () async {
                                                  await DbController.updateFav(
                                                    name: snapshot.data!.trucking!.name.toString(),
                                                    item: TrifsDB(
                                                        id: itemId,
                                                        type: _type,
                                                        image: snapshot.data!.trucking!.image!,
                                                        title: snapshot.data!.trucking!.name.toString(),
                                                        desc: snapshot.data!.trucking!.desc,
                                                        fav: true),
                                                  );
                                                  ...
                                                  // print(itemId);
                                                  // print(_type);
                                                  // print(snapshot.data!.trucking!.image!);
                                                  // print(snapshot.data!.trucking!.name.toString());
                                                  DbController.updateIcon(name: snapshot.data!.trucking!.name!);
                                                  setState(() {});
                                    ...
                                    Positioned(
                                      ...
                                      child: Container(
                                        ...
                                        child: Center(
                                          child: Text(
                                            'Price Starts from ₹${snapshot.data!.trucking!.offerAmount}/-',
                                                        ...
                                                        child: Padding(
                                                          ...
                                                          child: Column(
                                                            ...
                                                            children: [
                                                              ...
                                                              SizedBox(height: 10),
                                                              Text(
                                                                '${snapshot.data!.trucking!.inclusion}',
                                                                style: TextStyle(fontSize: 13),
                                                              ),
                                                              SizedBox(
                                                                height: 10,
                                                              ...
                                                              SizedBox(height: 10),
                                                              Text(
                                                                '${snapshot.data!.trucking!.exclusion}',
                                                                style: TextStyle(fontSize: 13),
                                                              ),
                                                              SizedBox(height: 30),
                                                            ],
                                                                ...
                                                                itemBuilder: (BuildContext context, int index) {
                                                                  return Padding(
                                                                    ...
                                                                    child: snapshot.data!.images!.isEmpty || snapshot.data!.images == null
                                                                        ...
                                                                        : GestureDetector(
                                                                            onTap: () => Get.toNamed('/fullscreenimage',
                                                                                arguments: snapshot.data!.images![index].image),
                                                                            child: Container(
                                                                              // width: MediaQuery.of(context).size.width * 0.25,
                                                                              height: MediaQuery.of(context).size.height * 0.10,
                          ...
                          children: [
                            GestureDetector(
                              onTap: () {
                                ...
                                enquiryMethod(
                                    type: 'Enquiry',
                                    packageType: 'Trucking',
                                    packageId: '${snapshot.data!.trucking!.id}',
                                    agencyId: '${snapshot.data!.trucking!.agencyId}',
                                    customerId: '111');
                                // succes show dialog
                                showDialog(
                                    context: context,
