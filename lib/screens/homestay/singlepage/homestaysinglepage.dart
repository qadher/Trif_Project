...
Future<HomeStaySinglePageModel> fetchSinglePage(int id) async {
  final response = await http.post(Uri.parse(Api.apiUrl + 'homestay/$id'));

// Appropriate action depending upon the

// server response
  if (response.statusCode == 200) {
    return HomeStaySinglePageModel.fromJson(json.decode(response.body));
  } else {
...
Future<HouseBoatGalleryModel> fetchGallery(int id) async {
  final response =
      await http.post(Uri.parse(Api.apiUrl + 'homestay/gallery/$id'));
...
class HomeStaySinglePagePackage extends StatefulWidget {
  ...
  @override
  _HomeStaySinglePagePackageState createState() =>
      _HomeStaySinglePagePackageState();
}
...
class _HomeStaySinglePagePackageState extends State<HomeStaySinglePagePackage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Center(
        child: FutureBuilder<HomeStaySinglePageModel>(
                          ...
                          child: Column(
                            children: [
                              Stack(
                                clipBehavior: Clip.none,
                                children: [
                                  snapshot.data!.homestay!.image!.isEmpty ||
                                          snapshot.data!.homestay!.image == null
                                      ? Container(
                                          height: 500,
                                        ...
                                        child: Row(
                                          ...
                                          children: [
                                            ...
                                            InkWell(
                                             ...
                                             await DbController.updateFav(name: snapshot.data!.homestay!.name.toString(), item: TrifsDB(id: itemId, type: _type, image: snapshot.data!.homestay!.image!, title: snapshot.data!.homestay!.name.toString(), desc: snapshot.data!.homestay!.desc!, fav: true),);
                                                final data = RepositoryBox.getBox();
                                                print(DbController.observableBox.values.toList());
                                                print(DbController.observableBox.length);
                                                print(itemId);
                                                print(_type);
                                                print(snapshot.data!.homestay!.image!);
                                                print(snapshot.data!.homestay!.name.toString());
                                                DbController.updateIcon(name: snapshot.data!.homestay!.name!);
                                                setState(() {
                                  ...
                                  Positioned(
                                    ...
                                    child: Container(
                                      ...
                                      child: Center(
                                        child: Text(
                                          'Price Starts from ₹${snapshot.data!.homestay!.offerAmount}/-',
                                          style: TextStyle(
                                              fontSize: 22,
                                              fontWeight: FontWeight.bold,
                                              color: Colors.white),
